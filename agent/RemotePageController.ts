...
export class RemotePageController {
	...
	async clickElementAtOffset(index: number, xPercent: number, yPercent: number): Promise<DomActionReturn> {
		return this.remoteCallDomAction('click_element_at_offset', [index, xPercent, yPercent])
	}

	/**
	 * Capture a screenshot of the current visible tab for VLM analysis.
	 * Routes through background worker via chrome.tabs.captureVisibleTab().
	 * Returns a base64 data URL (JPEG, 70% quality) or null on failure.
	 */
	async captureScreenshot(): Promise<string | null> {
		if (!this.currentTabId) {
			console.warn('[RemotePageController] Cannot capture screenshot: no active tab.');
			return null;
		}
		try {
			...
			console.warn('[RemotePageController] Screenshot capture failed:', response?.error);
			return null;
		} catch (err) {
			console.error('[RemotePageController] Screenshot capture error:', err);
			return null;
		}
	}
